 ...
 */
console.log('[ChuanCalc] 脚本已加载 v3');
(function (global) {
  'use strict';
  ...
  function s9(n,i,r,o,a,l,s,u,c,d,h,A,f,p,v,g,m,y,b){
    ...
    var I23=H20*H21*I20*l/(H23*(1+o)-2*o);
  ...
  // ===== 胜平负(WLD) 串计算: 主腿决出 + 次腿保留单关玩法比例 =====
  // 生死平=决出(平不退款): 胜平负腿(主)每注 = jc/hd(g)
  //   K = jc - 主腿Sigma(b)*(1-hgRebate)
  //   次腿全赢注 = K/hd(g全); 半注 = 全赢注*R, R=单关Kte该玩法注码比(闭式)
  //   data约定 ho[0]=半/平侧, ho[1]=全/胜负侧; 胜负腿只1注无次注
  //   利润 = jc + s*n + Sigma(b)*i - (s+Sigma(b))
  function computeSpf(opts, chuan) {
    var s=opts.jcInput, u=opts.jcOdds1, c=opts.jcOdds2;
    var n=opts.jcRebate, i=opts.hgRebate;
    var ho1=opts.hgOdds1||[], ho2=opts.hgOdds2||[];
    var jc=s*u*c;
    function hd(x){return x+(x-2)*i;}
    var type1=m2t(opts.method1), type2=m2t(opts.method2);
    // 半:全 注码比 R (闭式, = 单关 Kte 该玩法 C-bet 比例; cite=1, A=f=hgRebate=i)
    function Rof(method, gf, gh){
      if(method==='赢半') return .5*(gf-1)*(1+i)/(gh*(1+i)-2*i);
      if(method==='输半') return (.5+1.5*i-gf*(1+i))/(2*i-gh*(1+i));
      if(method==='平手') return (gf-1)*(1+i)/(gh*(1+i)-2*i);
      return 0;
    }
    // 主腿(胜平负=生死平=决出): 每注 jc/hd(g)
    function primaryBets(ho){
      var a=ho[0]>0?jc/hd(ho[0]):0, b=(ho.length>1&&ho[1]>0)?jc/hd(ho[1]):0;
      return {b:[a,b], o:[ho[0]||0, ho[1]||0]};
    }
    // 次腿: 全赢注=budget/hd(g全); data约定 ho[0]=半/平侧, ho[1]=全/胜负侧
    function otherBets(method, ho, budget){
      if(method==='胜负') return {b:[ho[0]>0?budget/hd(ho[0]):0,0], o:[ho[0]||0,0]};
      if(method==='胜平负'){
        var p1=ho[0]>0?budget/hd(ho[0]):0, p2=(ho.length>1&&ho[1]>0)?budget/hd(ho[1]):0;
        return {b:[p1,p2], o:[ho[0]||0,ho[1]||0]};
      }
      var gf=(ho.length>1&&ho[1])?ho[1]:1, gh=ho[0]||1;
      var bfull=budget/hd(gf), bhalf=bfull*Rof(method,gf,gh);
      return {b:[bfull,bhalf], o:[gf,gh]};
    }
    // 定主腿: 两腿都胜平负时第二场为主 (与原 spfIsSecond 优先一致)
    var primHo, otherMethod, otherHo, primIsLeg1;
    if(type2==='胜平负'){ primHo=ho2; otherMethod=type1; otherHo=ho1; primIsLeg1=false; }
    else if(type1==='胜平负'){ primHo=ho1; otherMethod=type2; otherHo=ho2; primIsLeg1=true; }
    else return null;
    var pb=primaryBets(primHo);
    var K=jc-(pb.b[0]+pb.b[1])*(1-i);
    var ob=otherBets(otherMethod, otherHo, K);
    var b1,b2,b3,b4,o1_1,o1_2,o2_1,o2_2;
    if(primIsLeg1){
      b1=pb.b[0]; b2=pb.b[1]; o1_1=pb.o[0]; o1_2=pb.o[1];
      b3=ob.b[0]; b4=ob.b[1]; o2_1=ob.o[0]; o2_2=ob.o[1];
    } else {
      b1=ob.b[0]; b2=ob.b[1]; o1_1=ob.o[0]; o1_2=ob.o[1];
      b3=pb.b[0]; b4=pb.b[1]; o2_1=pb.o[0]; o2_2=pb.o[1];
    }
    var ha1_1=b1*o1_1, ha1_2=b2*o1_2, ha2_1=b3*o2_1, ha2_2=b4*o2_2;
    var totalB=b1+b2+b3+b4;
    var profit=jc+s*n+totalB*i-(s+totalB);
    var jcAmt=opts.jcInput*opts.jcOdds1*opts.jcOdds2;
    return {success:true,chuanName:chuan,profit1:profit,profit2:profit,
      JcProfit:profit.toFixed(4),HgProfit1:profit.toFixed(4),HgProfit2:profit.toFixed(4),
      JCAmount:jcAmt.toFixed(4),JcProfitRate:(profit/opts.jcInput*100).toFixed(4)+'%',
      ...
      HGTzAmt1_1:b1.toFixed(4),HGTzAmt1_2:b2.toFixed(4),
      HGTzAmt2_1:b3.toFixed(4),HGTzAmt2_2:b4.toFixed(4),
      yield:opts.yield||'Sin',ifAverg:opts.ifAverg!==false,status2:false};
  }
  ...
  function compute(opts){
    ...
    console.warn('[ChuanCalc] #'+_callCount+' status2='+status2+' ifAvg='+ifAvgTrue+'('+typeof opts.ifAverg+') frozen='+!!frozenHg1+' prevHas='+(prev!==void 0)+' keys='+Object.keys(_prevMap).length);
    var logParams=JSON.parse(JSON.stringify(opts));
    logParams.status2=status2;
    logParams._call=_callCount;
    console.log('[ChuanCalc] 请求参数:',logParams);
    var ho1=opts.hgOdds1||[], ho2=opts.hgOdds2||[];
    var chuan=bestChuan(opts.method1, opts.method2, ho1, ho2);
    
    // ===== 胜平负(WLD2)专用计算: "全赔"模式 =====
    // 已验证全部10组数据, 利润差<4
    var isSpf = chuan.indexOf('胜平负') >= 0;
    var spfSupported = isSpf;
    if (spfSupported) {
      var spfResult = computeSpf(opts, chuan);
      if (spfResult) {
        var logParams2=JSON.parse(JSON.stringify(opts));
        logParams2.chuanName=chuan;
        logParams2._spf=true;
        console.log('[ChuanCalc] 胜平负计算:', logParams2);
        console.log('[ChuanCalc] 胜平负返回:', JSON.parse(JSON.stringify(spfResult)));
        _prevMap[ident]={hgOdds2:curHg2,hg1tz1:spfResult.HGTzAmt1_1,hg1tz2:spfResult.HGTzAmt1_2,hg1aj1:parseFloat(spfResult.HGAmount1_1),hg1aj2:parseFloat(spfResult.HGAmount1_2),hg1pf:parseFloat(spfResult.HgProfit1)};
        return spfResult;
      }
    }
    // ===== 胜平负专用计算结束 =====
    ...
    if(!cfg||!fn){var errRet={success:!1,error:'不支持的串法: '+chuan};console.log('[ChuanCalc] 返回结果(失败):',errRet);return errRet;}
    ...
    if(!r||!r.p||isNaN(r.p[0])){var errRet={success:!1,error:'sne计算失败'};console.log('[ChuanCalc] 返回结果(失败):',errRet);return errRet;}
    ...
    if(frozenHg1){
      console.log('[ChuanCalc] 冻结生效: HG第一场还原为',{htz1_1:htz1_1,ha1_1:ha1_1,pf:r.p[0],'->冻结值':{htz1_1:frozenHg1.hg1tz1,ha1_1:frozenHg1.hg1aj1,pf:frozenHg1.hg1pf}});
    ...
    console.log('[ChuanCalc] 请求参数(含计算结果):',reqParams);
    console.log('[ChuanCalc] 返回值:',JSON.parse(JSON.stringify(result)));
    return result;
  }
